...
### Процесс сборки
1. **Установка библиотеки NODRIVERJmini**
   ```bash
   git clone git@github.com:VityazevEgor/NODRIVERJmini.git
   cd NODRIVERJmini/nodriverjmini/
   bash install.sh
   ```

2. **Сборка и установка библиотеки:**
   ```bash
   cd llmapi4free_lib
   bash install.sh
   ```
